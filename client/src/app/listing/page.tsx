...
import { collections, categories, sizes } from "@/utils/config";
...
function ProductListingPage() {
  ...
  const handleToggleFilter = (
    filterType: "categories" | "sizes" | "collections" | "colors",
    value: string
  ) => {
    const setterMap = {
            ...
            {collections.map((collection) => (
              <div key={collection} className="flex items-center">
                <Checkbox
                  ...
                  onCheckedChange={() =>
                    handleToggleFilter("collections", collection)
                  }
                  id={collection}
                />
                <Label htmlFor={collection} className="ml-2 text-sm">
                  {collection}
                </Label>
              </div>
            ))}
